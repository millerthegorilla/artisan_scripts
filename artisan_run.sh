...
while (( "$#" )); do
  case "$1" in
    ...
    settings)
      ...
      runuser --login ${USERNAME} -P -c "podman exec -e PROJECT_NAME=${PROJECT_NAME} -it ${DJANGO_CONT_NAME} bash -c \"chown artisan:artisan /etc/opt/${PROJECT_NAME}/settings/settings.py\""
      exit $?
      ;;
    interact)
      if [[ -z ${USER_NAME} ]]
