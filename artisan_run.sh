...
while (( "$#" )); do
  case "$1" in
    ...
    tests_on)
      ...
      runuser --login ${USER_NAME} -P -c "podman exec -it ${MARIA_CONT_NAME} bash -c  \"echo 'grant all on *.* to ${DB_USER}@${DB_HOST}' | mysql -uroot -p${ROOT_PWD}\""
      exit $?
      ;;
    tests_off)
      if [[ -z ${MARIA_CONT_NAME} ]]
      ...
      then
        ...
        runuser --login dev -P -c "podman exec -it mariadb_cont bash -c \"echo 'GRANT CREATE, ALTER, INDEX, SELECT, UPDATE, INSERT, DELETE ON ${DB_NAME}.* TO ${DB_USER}@${DB_HOST} IDENTIFIED BY \\\"${DB_PASSWORD}\\\";' | mysql -uroot -p${ROOT_PWD}\""
      fi
      exit $?
      ;;
    help|-h|-?|--help)
