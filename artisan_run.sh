...
while (( "$#" )); do
  case "$1" in
    ...
    pip)
      shift;
      COMMANDS="$*"
      runuser --login ${USER_NAME} -P -c "podman exec -e PROJECT_NAME=${PROJECT_NAME} -it ${DJANGO_CONT_NAME} bash -c \"runuser -u artisan -c source /home/artisan/django_venv/bin/activate && pip ${COMMANDS}\""
      exit $?
      ;;
    help|-h|-?|--help)
