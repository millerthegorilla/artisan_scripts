...
if [[ ! -f ${SWAG_HOST_LOG_DIR} ]]
then
    mkdir -p ${SWAG_HOST_LOG_DIR}
fi

podman run -dit --pod=${POD_NAME} --name=${SWAG_CONT_NAME} --cap-add=NET_ADMIN -e PUID=1000 -e PGID=1000 -e TZ=Europe/London -e URL=${DUCKDNS_SUBDOMAIN}  -e VALIDATION=duckdns -e DUCKDNSTOKEN=${DUCKDNS_TOKEN} -e EXTRA_DOMAINS=${EXTRA_DOMAINS} -v ${SWAG_HOST_VOL_STATIC}:${SWAG_CONT_VOL_STATIC}:Z -v ${SWAG_HOST_LOG_DIR}:${SWAG_CONT_LOG_DIR}:Z --restart unless-stopped ${SWAG_IMAGE}
