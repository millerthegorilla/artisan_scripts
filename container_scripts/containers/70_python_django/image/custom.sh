...
function build_django()
{
  ...
  runuser --login ${USER_NAME} -c "podman build --build-arg=PROJECT_NAME=${PROJECT_NAME} --build-arg=STATIC_DIR=${DJANGO_HOST_STATIC_VOL} --build-arg=MEDIA_DIR=${DJANGO_HOST_MEDIA_VOL} --tag=\"${custom_tag}\" -f=/home/${USER_NAME}/django/${1} /home/${USER_NAME}/django/"
